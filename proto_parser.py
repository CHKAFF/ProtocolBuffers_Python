        ...
        while len(self.file_lines) > 1:
            for i in range(len(self.file_lines)):
                ...
                if message != None:
                    ...
                    self.classes.append(self.parse_new_class(message[1], 0))
                    break
        return self.classes
            
    
    def parse_new_class(self, name, class_level):
        new_class = {"name" : name, "properties" : [], "enums" : [], "classes" : []}
        exit_flag = True
        ...
        while exit_flag:
            for j in range(len(self.file_lines)):
                ...
                if search != None:
                    self.file_lines = self.file_lines[j+1:]
                    new_class["classes"].append(self.parse_new_class(search[1], class_level+1))
                    break
                ...
                if search != None:
                    new_class["properties"].append({"modifier" :  search[1], "type" : search[2], "name" : search[3], "value" : search[4]})
